...
def updatePaddles(win, keys):
    ...
    if numPlayers == 1:
        rightPaddle.CPU_move(gameBall, 2)
    elif numPlayers == 2:
...
def startMenu(win):
    ...
    while startMenu:
        ...
        startButton = Button((255, 255, 0), 200, 300, 100, 50) #(x,y) = 225, 317.5
        startButton.draw(win, "Start", 225, 317.5)
...
def numPlayerInput(win):
    ...
    while numPlayerInput:
        ...
        numPlayersText = numPlayersFont.render("How many players?", 1, (0, 0, 0))
        ...
        for event in pygame.event.get():
            if (event.type == pygame.QUIT
                    or event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE):
                quit()
            if event.type == pygame.KEYDOWN and (event.key == pygame.K_SPACE or event.key == pygame.K_1):
                numPlayers = 1
...
def main():
    ...
    startMenu(win)
    global numPlayers
    numPlayers = numPlayerInput(win)
    
    #Initialize the game ball and the paddles
    global gameBall
    global leftPaddle
    global rightPaddle
    gameBall = Ball((0, 255, 0), 490, 180, 15, 10, -10, True)
    leftPaddle = Paddle((255, 255, 255), 40, 450, 0, 15, 10, 65)
    rightPaddle = Paddle((255, 255, 255), 460 - 10, 150, 0, 15, 10, 65)
    ...
    while running:
        ...
        leftPaddle.move(keys, 1)
        if numPlayers == 1:
            rightPaddle.CPU_move(gameBall, None)
        elif numPlayers == 2:
            rightPaddle.move(keys, 2)
        updateScoreboard(win, player1Score, player2Score)
